...
def fetch_vinted(query: str) -> list:
    global _session
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0.0.0 Safari/537.36",
...
def calculer_marge(prix, marque):
...
def analyser(item):
    ...
    revente, marge = calculer_marge(prix, marque)
    if marge < marge_min:
        return False, {}
...
#  SCAN ASYNC
# ══════════════════════════════════════════════════════════════════════════════
async def check_vinted(app: Application):
    print(f"\n🔍 Scan — {time.strftime('%H:%M:%S')} | cooldown {config['cooldown']}min")
    alertes = 0

    for query in SEARCH_QUERIES:
        if not config["actif"]:
            break
        items = await asyncio.get_event_loop().run_in_executor(None, fetch_vinted, query)
    ...
    print(f"✅ Scan terminé — {alertes} alertes envoyées")

async def boucle_scan(app: Application):
    while True:
        if config["actif"]:
            await check_vinted(app)
        await asyncio.sleep(config["cooldown"] * 60)
...
async def cmd_stop(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    config["actif"] = False
    await update.message.reply_text("⛔ <b>Bot mis en pause.</b>\nTape /start pour relancer.", parse_mode="HTML")
...
async def cmd_cooldown(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(f"⏱️ Cooldown : <b>{minutes} minutes</b>", parse_mode="HTML")
    except Exception:
...
async def post_init(app: Application):
    """Lancé après l'init de l'application — démarre la boucle de scan."""
    ...
    await app.bot.send_message(
        ...
        text=(
            ...
            "/cooldown 10 — changer l'intervalle\n"
